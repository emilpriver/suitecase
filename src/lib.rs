...
//! | [`test_suite!`](crate::test_suite) | Emit one `#[test]` per case; all share one `Mutex` suite and run in registration order via a shared cursor (optional inline [`cases!`] syntax so names are not repeated) |
//!
//! # Assertions
//!
